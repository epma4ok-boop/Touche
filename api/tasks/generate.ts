...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { category = "compliments", lang = "ru", gender } = (req.body ?? {}) as {
    category?: string;
    lang?: string;
    gender?: string;
  };

  // No API key — return static fallback immediately
  if (!DEEPSEEK_API_KEY) {
  ...
  // Gender-specific prompts (desire, passion, hard)
  const genCat = GENDERED[category];
  if (genCat) {
    const langMap = genCat[lang] ?? genCat["en"] ?? genCat["ru"];
    if (langMap) {
      systemPrompt = gender === "female" ? langMap.female : langMap.male;
    }
  }

  // Neutral prompts (compliments, tenderness) — append gender note
  if (!systemPrompt) {
    const neutralCat = NEUTRAL[category];
    if (neutralCat) {
      systemPrompt = neutralCat[lang] ?? neutralCat["en"] ?? neutralCat["ru"];
      if (systemPrompt && gender) {
        const genderNotes: Record<string, Record<string, string>> = {
          ru: {
            male:   "\n\nПОЛЬЗОВАТЕЛЬ — МУЖЧИНА. Партнёрша — ЖЕНЩИНА. Используй мужской род для пользователя.",
            female: "\n\nПОЛЬЗОВАТЕЛЬ — ЖЕНЩИНА. Партнёр — МУЖЧИНА. Используй женский род для пользователя.",
          },
          en: {
            male:   "\n\nUSER IS A MAN. Partner is a WOMAN.",
            female: "\n\nUSER IS A WOMAN. Partner is a MAN.",
          ...
          hi: {
            male:   "\n\nउपयोगकर्ता पुरुष है। पार्टनर महिला है।",
            female: "\n\nउपयोगकर्ता महिला है। पार्टनर पुरुष है।",
          },
          pt: {
            male:   "\n\nUSUÁRIO É HOMEM. Parceira é MULHER.",
            female: "\n\nUSUÁRIA É MULHER. Parceiro é HOMEM.",
        ...
        const note = genderNotes[lang]?.[gender] ?? genderNotes["en"]?.[gender];
  ...
  try {
    ...
    if (!response.ok) {
      console.error("DeepSeek error:", response.status, await response.text().catch(() => ""));
      return res.status(200).json({ task: getFallback(category, lang), source: "fallback" });
    }
    ...
    // Filter AI self-referential phrases
    const lower = raw.toLowerCase();
    if (FORBIDDEN.some((p) => lower.includes(p))) {
      return res.status(200).json({ task: getFallback(category, lang), source: "fallback" });
    }

    return res.status(200).json({ task: raw, source: "ai" });
  } catch (err) {
