...
async function tryFetchPendingScenario(): Promise<ActiveScenario | null> {
  try {
    ...
    const res = await fetch(`/api/scenario/pending?coupleId=${encodeURIComponent(coupleId)}`, {
...
async function tryFetchScenarioSession(sessionId: string): Promise<ActiveScenario | null> {
  try {
    ...
    const res = await fetch(`/api/scenario/session?id=${encodeURIComponent(sessionId)}`, {
      headers: { "x-telegram-init-data": initData },
    });
    if (!res.ok) return null;
    const data = await res.json();
