  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      for (let i = ringCount - 1; i >= 0; i--) {
        ...
        // ── BRIGHTNESS BOOST: base alpha значительно увеличен ──
        const baseAlpha = (1 - spreadPhase) * (0.52 + holdProgress * 0.42) * (1 + beatEnv * 0.60);
        const alpha = Math.min(0.95, baseAlpha + burst * (1 - spreadPhase) * 0.65);
      ...
      // ── Glow значительно ярче ──
      const glowR = baseR * (0.9 + beatEnv * 0.22 + holdProgress * 0.38 + burst * 0.50);
      const glow = ctx.createRadialGradient(cx, cy, 0, cx, cy, glowR);
      const ga = 0.30 + holdProgress * 0.42 + beatEnv * 0.20 + burst * 0.40;
      ...
      // ── Центральная точка ярче ──
      const dotR = baseR * (0.28 + beatEnv * 0.09 + holdProgress * 0.11 + burst * 0.20);
      const dotG = ctx.createRadialGradient(cx, cy, 0, cx, cy, dotR);
      dotG.addColorStop(0, `rgba(${r},${g},${b},${Math.min(1, 0.88 + holdProgress * 0.12 + beatEnv * 0.10)})`);
      dotG.addColorStop(0.6, `rgba(${r},${g},${b},${0.45 + holdProgress * 0.25})`);
      ...
      const hintAlpha = holdActiveRef.current
        ? Math.max(0, 1 - holdProgress * 2.5) : 0.65;
      if (hintAlpha > 0.01) {
        ctx.save();
