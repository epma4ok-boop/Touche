  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      for (let i = ringCount - 1; i >= 0; i--) {
        ...
        const baseAlpha = (1 - spreadPhase) * (0.16 + holdProgress * 0.24) * (1 + beatEnv * 0.45);
        const alpha = Math.min(0.80, baseAlpha + burst * (1 - spreadPhase) * 0.45);
        const isBlue = i % 2 === 0;
        const cr = isBlue ? 110 : r;
        const cg = isBlue ? 175 : g;
        const cb = isBlue ? 235 : b;
        ctx.beginPath();
      ...
      const ga = 0.10 + holdProgress * 0.18 + beatEnv * 0.10 + burst * 0.28;
      glow.addColorStop(0, `rgba(${r},${g},${b},${ga})`);
      glow.addColorStop(0.5, `rgba(110,175,235,${ga * 0.4})`);
      ...
      dotG.addColorStop(0, `rgba(${r},${g},${b},${0.50 + holdProgress * 0.38 + beatEnv * 0.12})`);
      dotG.addColorStop(0.6, `rgba(110,175,235,${0.22 + holdProgress * 0.18})`);
      ...
      const hintAlpha = holdActiveRef.current
        ? Math.max(0, 1 - holdProgress * 2.5) : 0.42;
      if (hintAlpha > 0.01) {
        ctx.save();
        ctx.globalAlpha = hintAlpha;
        const fs = Math.min(10, Math.max(8, w * 0.022));
        ctx.font = `300 ${fs}px 'Plus Jakarta Sans','DM Sans',sans-serif`;
        ctx.fillStyle = `rgba(${r},${g},${b},1)`;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(hintRef.current.toUpperCase(), cx, cy + Math.min(w, h) * 0.36);
        ctx.restore();
      }
