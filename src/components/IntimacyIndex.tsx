...
function HistoryChart({ history }: { history: IntimacyLocal["history"] }) {
  ...
  const max = Math.max(...last7.map(h => h.points), 1);
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-end", gap: 5, height: 52, padding: "0 2px" }}>
      {last7.map((h, i) => {
        const hp  = (h.points / max) * 100;
        const isT = h.date === today; const isY = h.date === yest;
        const label = isT ? "·" : isY ? "·" : h.date.slice(8);
        return (
          <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
            <div style={{
              width: "100%", borderRadius: 4,
              height: `${Math.max(8, hp * 0.44)}px`,
              background: isT
                ? `linear-gradient(180deg,rgba(${PR},${PG},${PB},0.9),rgba(${PR},${PG},${PB},0.5))`
                : "rgba(255,238,248,0.12)",
              boxShadow: isT ? `0 0 8px rgba(${PR},${PG},${PB},0.5)` : "none",
              transition: "height .8s cubic-bezier(.22,1,.36,1)",
            }} />
            <span style={{ fontSize: 9, color: isT ? PINK : "rgba(255,238,248,0.28)", fontFamily: "'Plus Jakarta Sans',sans-serif" }}>{label}</span>
          </div>
        );
      })}
    </div>
...
function IntimacyModal({ lang, data, onClose }: { lang: Lang; data: IntimacyLocal; onClose: () => void }) {
  const t    = getT(lang);
  const lvl  = getLevel(data.score);
  const prog = getLevelProgress(data.score);
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 900, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
      <div onClick={onClose} style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.70)", backdropFilter: "blur(8px)" }} />
      <div style={{
        position: "relative", zIndex: 1,
        background: "linear-gradient(180deg,#1a0a1e 0%,#110810 100%)",
        borderRadius: "28px 28px 0 0",
        padding: "0 22px max(34px,env(safe-area-inset-bottom))",
        border: `1px solid rgba(${PR},${PG},${PB},0.20)`, borderBottom: "none",
        boxShadow: `0 -10px 60px rgba(${PR},${PG},${PB},0.20)`,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20, marginBottom: 22, paddingTop: 8 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 800, fontSize: 40, color: "rgba(255,238,248,0.97)", lineHeight: 1, letterSpacing: "-0.03em" }}>{score}</div>
            <div style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 12, color: "rgba(255,238,248,0.38)", marginTop: 2, letterSpacing: "0.06em", textTransform: "uppercase" }}>{t.title}</div>
            <div style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 700, fontSize: 15, color: lvl.color, marginTop: 5, textShadow: `0 0 12px ${lvl.color}` }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
          {[
            { icon: "🔥", main: data.streakDays > 0 ? t.streak(data.streakDays) : t.noStreak },
            { icon: "✓",  main: t.tasks(data.totalTasks) },
          ].map((item, i) => (
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, color: "rgba(255,238,248,0.32)", letterSpacing: "0.06em", textTransform: "uppercase" }}>
              {nextLvl ? t.progress : t.maxLevel}
            </span>
            {nextLvl && (
              <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 12, color: lvl.color }}>
                {nextLvl.icon} {lang === "ru" ? nextLvl.nameRu : nextLvl.nameEn}
              </span>
            )}
          </div>
        ...
        {/* Level map */}
        <div style={{ marginBottom: 20 }}>
          <div style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, color: "rgba(255,238,248,0.32)", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 10 }}>
            ...
            {LEVELS.map((l, i) => {
              ...
              return (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 14px", borderRadius: 12, background: active ? `rgba(${PR},${PG},${PB},0.12)` : "transparent", border: `1px solid ${active ? `rgba(${PR},${PG},${PB},0.28)` : "transparent"}`, transition: "all .3s" }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: active ? 700 : 400, fontSize: 13, color: active ? l.color : "rgba(255,238,248,0.38)" }}>
                      {lang === "ru" ? l.nameRu : l.nameEn}
                    </span>
                  </div>
                  <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, color: "rgba(255,238,248,0.20)" }}>
...
export default function IntimacyIndex({ lang, refreshKey = 0, index = 0 }: IntimacyIndexProps) {
  ...
  // Reload when task is completed (refreshKey bumped via CustomEvent in Home)
  useEffect(() => { setData(loadLocal()); }, [refreshKey]);

  // Also listen for the event directly
  useEffect(() => {
    const handler = () => setData(loadLocal());
    window.addEventListener("touche-intimacy-updated", handler);
    return () => window.removeEventListener("touche-intimacy-updated", handler);
  }, []);

  const score = useCountUp(data.score, 700);
  const lvl   = getLevel(data.score);
  const prog  = getLevelProgress(data.score);
  const t     = getT(lang);
    ...
    <>
      <button
        ...
        style={{
          ...
          background: `linear-gradient(135deg,rgba(${PR},${PG},${PB},0.11) 0%,rgba(${PR},${PG},${PB},0.04) 100%)`,
          border: `1px solid rgba(${PR},${PG},${PB},0.24)`,
          borderRadius: 22, padding: "15px 18px",
          animation: `fadeSlideUp .42s cubic-bezier(.22,1,.36,1) ${index * 55}ms both`,
          boxShadow: `0 0 28px rgba(${PR},${PG},${PB},0.09)`,
          position: "relative", overflow: "hidden",
        }}
      >
        {/* Glow orb */}
        <div style={{ position: "absolute", top: -30, right: -20, width: 120, height: 120, borderRadius: "50%", background: `radial-gradient(circle,rgba(${PR},${PG},${PB},0.14) 0%,transparent 70%)`, pointerEvents: "none" }} />

        <div style={{ display: "flex", alignItems: "center", gap: 14, position: "relative" }}>
          {/* Ring */}
          <div style={{ position: "relative", flexShrink: 0 }}>
            <Ring progress={prog} color={lvl.color} size={56} />
            <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, fontWeight: 500, color: "rgba(255,238,248,0.38)", letterSpacing: "0.10em", textTransform: "uppercase", marginBottom: 2 }}>{t.title}</div>
            <div style={{ display: "flex", alignItems: "baseline", gap: 7 }}>
              <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 800, fontSize: 28, color: "rgba(255,238,248,0.96)", lineHeight: 1, letterSpacing: "-0.02em" }}>{score}</span>
              <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 600, fontSize: 13, color: lvl.color, textShadow: `0 0 10px ${lvl.color}` }}>{lang === "ru" ? lvl.nameRu : lvl.nameEn}</span>
            </div>
            {data.streakDays > 0 && (
              <div style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, color: "rgba(255,238,248,0.32)", marginTop: 3 }}>🔥 {t.streak(data.streakDays)}</div>
            )}
          </div>

          {/* Chevron */}
          <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke={`rgba(${PR},${PG},${PB},0.50)`} strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
            <polyline points="9 18 15 12 9 6"/>
          </svg>
        </div>

        {/* Progress bar */}
        <div style={{ marginTop: 11, height: 3, borderRadius: 99, background: "rgba(255,238,248,0.06)", overflow: "hidden" }}>
          <div style={{ height: "100%", borderRadius: 99, width: `${Math.round(prog*100)}%`, background: `linear-gradient(90deg,rgba(${PR},${PG},${PB},0.5),${lvl.color})`, boxShadow: `0 0 6px ${lvl.color}`, transition: "width 1s cubic-bezier(.22,1,.36,1)" }} />
        </div>
      </button>
