...
const LANGS: { lang: Lang; primary: string; sub: string; flag: string }[] = [
  { lang: "en", primary: "English",    sub: "Английский",  flag: "🇬🇧" },
  { lang: "ru", primary: "Русский",    sub: "Russian",      flag: "🇷🇺" },
  { lang: "hi", primary: "हिंदी",       sub: "Hindi",        flag: "🇮🇳" },
  { lang: "pt", primary: "Português",  sub: "Brazilian",    flag: "🇧🇷" },
  { lang: "es", primary: "Español",    sub: "Spanish",      flag: "🇪🇸" },
];
...
export default function LanguageSelect({ onSelect }: LanguageSelectProps) {
  ...
  const btnStyle = (isChosen: boolean): React.CSSProperties => ({
    flex: "1 1 calc(50% - 7px)",
    minWidth: 0,
    padding: "18px 8px",
    borderRadius: 18,
    background: isChosen ? "rgba(210,60,100,0.18)" : "rgba(255,238,248,0.06)",
    border: `1px solid ${isChosen ? "rgba(210,60,100,0.55)" : "rgba(210,60,100,0.22)"}`,
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "relative", zIndex: 2, display: "flex", flexDirection: "column", alignItems: "center", width: "100%", padding: "0 24px" }}>
        <p style={{
        ...
        {/* 2+2+1 grid */}
        <div style={{ display: "flex", flexWrap: "wrap", gap: 12, width: "100%", maxWidth: 340, justifyContent: "center" }}>
          {LANGS.map(({ lang, primary, sub, flag }) => (
            <button
              key={lang}
              onClick={() => pick(lang)}
              style={btnStyle(chosen === lang)}
              onMouseEnter={e => {
            ...
            >
              ...
              <span style={{ fontFamily: "'Plus Jakarta Sans','DM Sans',sans-serif", fontWeight: 600, fontSize: 16, color: "rgba(255,238,248,0.92)", letterSpacing: "-0.01em" }}>{primary}</span>
              <span style={{ fontFamily: "'Plus Jakarta Sans','DM Sans',sans-serif", fontWeight: 300, fontSize: 9, letterSpacing: "0.16em", textTransform: "uppercase", color: "rgba(210,80,120,0.60)" }}>{sub}</span>
            </button>
          ))}
        </div>

        <p style={{ fontFamily: "'Plus Jakarta Sans','DM Sans',sans-serif", fontWeight: 300, fontSize: 11, letterSpacing: "0.06em", color: "rgba(255,238,248,0.20)", marginTop: 28, textAlign: "center" }}>
