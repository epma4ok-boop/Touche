...
// ─── Local storage fallback (for non-linked users) ───────────────
export const INTIMACY_LOCAL_KEY = "touche_intimacy_v1";

export interface IntimacyLocal {
  score:            number;
  streakDays:       number;
  lastActivityDate: string;
  totalTasks:       number;
  history:          { date: string; points: number; total: number }[];
}
...
export function loadLocal(): IntimacyLocal {
  try {
    const v = localStorage.getItem(INTIMACY_LOCAL_KEY);
    if (v) return JSON.parse(v) as IntimacyLocal;
  } catch {}
...
export function addLocalPoints(category: Category): IntimacyLocal {
  ...
  // Decay: 5% per missed day, max 50 pts — applied lazily when user opens app
  let score = data.score;
  if (data.lastActivityDate && data.lastActivityDate < yest) {
    const daysMissed = Math.floor(
      (new Date(today).getTime() - new Date(data.lastActivityDate).getTime()) / 86400000
    ) - 1;
    for (let i = 0; i < Math.min(daysMissed, 30); i++) {
  ...
  let { streakDays } = data;
  if      (data.lastActivityDate === today) { /* same day */ }
  else if (data.lastActivityDate === yest)  { streakDays += 1; }
  else if (data.lastActivityDate === "")    { streakDays = 1; }
  else                                       { streakDays = 1; }

  const newScore = score + points;
  let history = [...data.history];
  const todayEntry = history.find(h => h.date === today);
  if (todayEntry) { todayEntry.points += points; todayEntry.total = newScore; }
  else            { history.push({ date: today, points, total: newScore }); }
  if (history.length > 30) history = history.slice(-30);

  const updated: IntimacyLocal = {
    score: newScore, streakDays, lastActivityDate: today,
    totalTasks: data.totalTasks + 1, history,
  };
