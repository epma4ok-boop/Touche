...
function StarsShop({ open, onClose, onPurchased, accentRgb, lang, paid }: StarsShopProps) {
  ...
  return (
    <>
      <div onClick={onClose} style={{ position:"absolute",inset:0,zIndex:40,background:"rgba(0,0,0,0.30)",opacity:open?1:0,pointerEvents:open?"auto":"none",transition:"opacity 0.3s",backdropFilter:open?"blur(4px)":"none" }} />
      <div style={{ position:"absolute",bottom:0,left:0,right:0,zIndex:50,borderRadius:"22px 22px 0 0",background:"rgba(255,252,248,0.98)",backdropFilter:"blur(32px)",borderTop:`1px solid rgba(${r},${g},${b},0.18)`,boxShadow:`0 -14px 50px rgba(0,0,0,0.12)`,transform:open?"translateY(0)":"translateY(110%)",transition:"transform 0.42s cubic-bezier(0.32,0.72,0,1)",paddingBottom:28 }}>
        <div style={{ display:"flex",justifyContent:"center",paddingTop:13,paddingBottom:5 }}>
          <div style={{ width:34,height:3,borderRadius:99,background:`rgba(${r},${g},${b},0.20)` }} />
        </div>
        <div style={{ textAlign:"center",padding:"8px 24px 22px" }}>
          <p style={{ fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:600,fontSize:22,color:"rgba(40,30,50,0.90)",margin:0,letterSpacing:"-0.01em" }}>{t.starsTitle}</p>
          <p style={{ fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:300,fontSize:13,color:`rgba(${r},${g},${b},0.70)`,margin:"8px 0 0",letterSpacing:"0.02em" }}>{t.starsSub}</p>
        </div>
        <div style={{ padding:"0 20px 14px" }}>
          <button onClick={handleBuy} disabled={loading} style={{ display:"flex",alignItems:"center",justifyContent:"space-between",width:"100%",padding:"17px 20px",borderRadius:16,background:`rgba(${r},${g},${b},0.07)`,border:`1px solid rgba(${r},${g},${b},${loading?0.5:0.22})`,cursor:loading?"default":"pointer",transition:"all 0.2s" }}>
            <span style={{ fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:500,fontSize:17,color:"rgba(40,30,50,0.85)",letterSpacing:"-0.01em" }}>{paid?t.starsPackPaid:t.starsPackFree}</span>
            <span style={{ fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:600,fontSize:17,color:`rgba(${r},${g},${b},0.9)`,flexShrink:0,marginLeft:12 }}>{loading?"...":(paid?t.starsPricePaid:t.starsPriceFree)}</span>
          </button>
        </div>
        <div style={{ padding:"0 20px" }}>
          <button onClick={onClose} style={{ width:"100%",padding:"12px 8px",borderRadius:14,background:"transparent",border:"1px solid rgba(40,30,50,0.10)",cursor:"pointer",fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:300,fontSize:13,letterSpacing:"0.06em",color:"rgba(40,30,50,0.35)" }}>{t.starsCancel}</button>
        </div>
      </div>
    </>
  );
        ...
        return (
          <button key={cat} onClick={() => onDotPress(cat)} style={{ background:"none",border:"none",padding:"6px 3px",cursor:"pointer",display:"flex",alignItems:"center" }}>
            <div style={{
              ...
              height: 3,
              borderRadius: 99,
              background: active ? `rgb(${cfg.r},${cfg.g},${cfg.b})` : "rgba(40,30,50,0.12)",
              boxShadow: active ? `0 0 8px rgba(${cfg.r},${cfg.g},${cfg.b},0.45)` : "none",
...
export default function CategoryScreen({ lang, category, onBack, onCategoryChange, swipeDir }: CategoryScreenProps) {
  ...
  const handleHoldComplete = useCallback(() => {
    ...
    const castDelay = 900 + Math.random() * 400;
    ...
    revealTimerRef.current = setTimeout(() => {
      ...
      let prog = 0;
      const step = () => { prog = Math.min(1, prog + 0.022); setRevealProgress(prog); if (prog < 1) requestAnimationFrame(step); };
  ...
  return (
    ...
    >
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 20px 6px",flexShrink:0,position:"relative",zIndex:10 }}>
        <button onClick={onBack} style={{ background:"transparent",border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:400,fontSize:13,letterSpacing:"0.01em",color:"rgba(40,30,50,0.38)",padding:"4px 0" }}>
          {t.backLabel}
        </button>
        <div style={{ textAlign:"center" }}>
          <p style={{ fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:600,fontSize:18,color:"rgba(40,30,50,0.85)",letterSpacing:"-0.01em",margin:0 }}>
            {catLabels[category]}
          </p>
        </div>
        <button onClick={() => setHistoryOpen(true)} style={{ background:"transparent",border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:400,fontSize:13,letterSpacing:"0.01em",color:"rgba(40,30,50,0.38)",padding:"4px 0" }}>
          {t.history}
        </button>
      </div>
      ...
      <div style={{ textAlign:"center",flexShrink:0,position:"relative",zIndex:10,padding:"0 24px 2px" }}>
        <p style={{ fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:300,fontSize:11,letterSpacing:"0.12em",textTransform:"uppercase",color:`rgba(${r},${g},${b},0.55)`,margin:0 }}>
          {category === "compliments" ? t.catComplimentsSub
            : category === "tenderness" ? t.catTendernessSub
            : category === "desire" ? t.catDesireSub
            : category === "passion" ? t.catPassionSub
            : t.catHardSub}
        </p>
      </div>
      ...
      {remaining > 0 && (
        <div style={{ textAlign:"center",flexShrink:0,position:"relative",zIndex:10,paddingTop:2 }}>
          <span style={{ fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:300,fontSize:10,letterSpacing:"0.10em",textTransform:"uppercase",color:`rgba(${r},${g},${b},0.35)` }}>
      ...
      <div style={{ flexShrink:0,position:"relative",zIndex:10,paddingBottom:4 }}>
        ...
        <p style={{ textAlign:"center",fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:300,fontSize:9,letterSpacing:"0.18em",textTransform:"uppercase",color:"rgba(40,30,50,0.15)",margin:"4px 0 0" }}>
          {t.swipeHint}
        </p>
      </div>
      ...
      <div style={{ display:"flex",justifyContent:"center",alignItems:"center",padding:"8px 20px max(16px,env(safe-area-inset-bottom))",flexShrink:0,position:"relative",zIndex:10 }}>
        {remaining === 0 ? (
          <button onClick={() => setStarsShopOpen(true)} style={{ background:"transparent",border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:300,fontSize:11,letterSpacing:"0.12em",textTransform:"uppercase",color:`rgba(${r},${g},${b},0.50)`,padding:"4px 12px" }}>
            {lang==="ru"?"ВЕРНИСЬ ЗАВТРА · ИЛИ КУПИ ★":"COME BACK TOMORROW · OR BUY ★"}
          </button>
        ) : (
          <span style={{ fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:300,fontSize:10,letterSpacing:"0.14em",textTransform:"uppercase",color:"rgba(40,30,50,0.12)" }}>touché</span>
        )}
      </div>
