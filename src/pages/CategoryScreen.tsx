      ...
      {CATEGORIES_ORDER.map(cat => {
        ...
        return (
          <button key={cat} onClick={() => onDotPress(cat)} style={{ background:"none",border:"none",padding:"6px 3px",cursor:"pointer",display:"flex",alignItems:"center" }}>
            <div style={{
              width: active ? 20 : 6, height: 3, borderRadius: 99,
              background: active ? `rgb(${cfg.r},${cfg.g},${cfg.b})` : "rgba(40,30,50,0.12)",
              boxShadow: active ? `0 0 8px rgba(${cfg.r},${cfg.g},${cfg.b},0.40)` : "none",
  ...
  return (
    ...
    >
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 20px 6px",flexShrink:0,position:"relative",zIndex:10 }}>
        <button onClick={onBack} style={{ background:"transparent",border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:400,fontSize:13,letterSpacing:"0.01em",color:"rgba(40,30,50,0.35)",padding:"4px 0" }}>
          {t.backLabel}
        </button>
        ...
        <button onClick={() => setHistoryOpen(true)} style={{ background:"transparent",border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:400,fontSize:13,color:"rgba(40,30,50,0.35)",padding:"4px 0" }}>
          {t.history}
        </button>
      </div>
      ...
      <div style={{ textAlign:"center",flexShrink:0,position:"relative",zIndex:10,padding:"0 24px 2px" }}>
        <p style={{ fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:300,fontSize:11,letterSpacing:"0.14em",textTransform:"uppercase",color:`rgba(${r},${g},${b},0.52)`,margin:0 }}>
          {catSubs[category]}
        </p>
      </div>
      ...
      {remaining > 0 && (
        <div style={{ textAlign:"center",flexShrink:0,position:"relative",zIndex:10,paddingTop:2 }}>
          <span style={{ fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:300,fontSize:10,letterSpacing:"0.10em",textTransform:"uppercase",color:`rgba(${r},${g},${b},0.32)` }}>
      ...
      <div style={{ flexShrink:0, position:"relative", zIndex:10, paddingBottom:4 }}>
        ...
        <p style={{ textAlign:"center",fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:300,fontSize:9,letterSpacing:"0.18em",textTransform:"uppercase",color:"rgba(40,30,50,0.14)",margin:"4px 0 0" }}>
          {t.swipeHint}
        </p>
      </div>
      ...
      <div style={{ display:"flex",justifyContent:"center",alignItems:"center",padding:"8px 20px max(16px,env(safe-area-inset-bottom))",flexShrink:0,position:"relative",zIndex:10 }}>
        {remaining === 0 ? (
          <button onClick={() => setStarsShopOpen(true)} style={{ background:"transparent",border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:300,fontSize:11,letterSpacing:"0.12em",textTransform:"uppercase",color:`rgba(${r},${g},${b},0.48)`,padding:"4px 12px" }}>
            {lang==="ru"?"ВЕРНИСЬ ЗАВТРА · ИЛИ КУПИ ★":"COME BACK TOMORROW · OR BUY ★"}
          </button>
        ) : (
          <span style={{ fontFamily:"'Plus Jakarta Sans','DM Sans',sans-serif",fontWeight:300,fontSize:10,letterSpacing:"0.14em",textTransform:"uppercase",color:"rgba(40,30,50,0.11)" }}>touché</span>
        )}
      </div>

      {/* Full-screen task reveal */}
      <TaskReveal
        text={taskText}
        color={cfg}
        visible={showReveal}
