...
async function createBonusInvoice(category: Category, lang: Lang): Promise<string | null> {
  try {
...
function StarsShop({ open, onClose, onPurchased, accentRgb, lang, category }: {
  ...
  const handleBuy = useCallback(async () => {
    setLoading(true);
    try {
      const invoiceLink = await createBonusInvoice(category, lang);
      if (invoiceLink) {
        window.Telegram?.WebApp?.openInvoice?.(invoiceLink, (status: string) => {
          if (status === "paid") { onPurchased(3); onClose(); }
          setLoading(false);
        });
      } else {
        setLoading(false);
  ...
  }, [category, lang, onPurchased, onClose]);
    ...
    <>
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, zIndex: 50, borderRadius: "22px 22px 0 0", background: "rgba(22,13,20,.99)", backdropFilter: "blur(32px)", borderTop: `1px solid rgba(${r},${g},${b},.28)`, boxShadow: "0 -14px 50px rgba(0,0,0,.40)", transform: open ? "translateY(0)" : "translateY(110%)", transition: "transform .42s cubic-bezier(.32,.72,0,1)", paddingBottom: 28 }}>
        ...
        <div style={{ padding: "0 20px 14px" }}>
          <button onClick={handleBuy} disabled={loading} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", padding: "17px 20px", borderRadius: 16, background: `rgba(${r},${g},${b},.10)`, border: `1px solid rgba(${r},${g},${b},.28)`, cursor: loading ? "default" : "pointer", opacity: loading ? 0.6 : 1 }}>
            <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 500, fontSize: 17, color: TEXT_P }}>{t.starsPackFree}</span>
            <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 600, fontSize: 17, color: `rgba(${r},${g},${b},.95)` }}>{loading ? "..." : t.starsPriceFree}</span>
          </button>
        </div>
...
function SubscriptionPaywall({ category, lang, onSubscribed }: {
  category: Category; lang: Lang; onSubscribed: () => void;
}) {
  const { r, g, b } = CATEGORY_CONFIG[category];
  const t = UI[lang];
  const [loading, setLoading] = useState(false);
  ...
  return (
    <div style={{
      ...
      {/* Title */}
      <p style={{
        fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 700, fontSize: 24,
      ...
      <p style={{
        fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 300, fontSize: 13,
      ...
      <div style={{
        ...
        {features.map((f, i) => (
          <div key={i} style={{
            ...
            <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 400, fontSize: 14, color: TEXT_P, lineHeight: 1.4 }}>{f}</span>
          </div>
        ))}
      </div>
      ...
      {/* Price note */}
      <p style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 300, fontSize: 11, letterSpacing: "0.08em", color: TEXT_S, margin: "0 0 4px", textAlign: "center" }}>{t.subPriceNote}</p>
    </div>
  );
}
...
export default function CategoryScreen({ lang, category, onBack, onCategoryChange, swipeDir }: Props) {
  ...
  const [isLocked, setIsLocked] = useState<boolean>(cfg.paid);
  const [subChecked, setSubChecked] = useState<boolean>(!cfg.paid);
  ...
  useEffect(() => {
    ...
    // Check limits + subscription in parallel
    const checkLimits = async () => {
      if (cfg.paid) {
        // Need subscription check first for paid categories
        const [sub, limitsData] = await Promise.all([
          fetchSubscriptionStatus(),
          fetchServerRemaining(category),
        ]);
        const locked = !sub.active;
        setIsLocked(locked);
        setSubChecked(true);
        if (!locked && limitsData !== null) setRemaining(limitsData.remaining);
      } else {
        const limitsData = await fetchServerRemaining(category);
  ...
  const handleBonusPurchased = useCallback((bonus: number) => {
    setRemaining(prev => prev + bonus);
  }, []);

  const handleSubscribed = useCallback(async () => {
    // Re-check subscription + limits after payment
    const [sub, limitsData] = await Promise.all([
      fetchSubscriptionStatus(),
      fetchServerRemaining(category),
    ]);
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: topPadding, paddingLeft: 20, paddingRight: 20, paddingBottom: 6, flexShrink: 0, position: "relative", zIndex: 10 }}>
        ...
        <div style={{ flex: 1 }} />
        {/* Premium badge if subscribed to paid category */}
        {cfg.paid && !isLocked && subChecked && (
      ...
      <div style={{ flex: 1, position: "relative", minHeight: 0 }}>
        {/* Subscription paywall for locked paid categories */}
        {cfg.paid && subChecked && isLocked ? (
          <SubscriptionPaywall category={category} lang={lang} onSubscribed={handleSubscribed} />
        ) : (
          <HeartbeatCanvas
      ...
      <StarsShop open={starsShopOpen} onClose={() => setStarsShopOpen(false)} onPurchased={handleBonusPurchased} accentRgb={cfg} lang={lang} category={category} />
    </div>
  );
}
