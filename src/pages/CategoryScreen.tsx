// CategoryScreen — original HeartbeatCanvas animation + AI tasks + server limits
      ...
      {CATEGORIES_ORDER.map(cat=>{
        ...
        return (
          <button key={cat} onClick={()=>onDotPress(cat)} style={{background:"none",border:"none",padding:"8px 4px",cursor:"pointer",display:"flex",alignItems:"center"}}>
            <div style={{width:active?24:7,height:4,borderRadius:99,background:active?`rgb(${cfg.r},${cfg.g},${cfg.b})`:"rgba(40,30,50,.15)",boxShadow:active?`0 0 10px rgba(${cfg.r},${cfg.g},${cfg.b},.45)`:"none",transition:"all .35s cubic-bezier(.32,.72,0,1)"}}/>
          </button>
        );
      })}
    </div>
...
function TaskReveal({ text,color,visible,onDismiss,lang,catLabel,source }: {
  ...
  return (
    <div style={{position:"absolute",inset:0,zIndex:30,background:`rgba(${r},${g},${b},.94)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 28px max(32px,env(safe-area-inset-bottom))",opacity:visible?1:0,pointerEvents:visible?"auto":"none",transition:"opacity .38s cubic-bezier(.22,1,.36,1)",backdropFilter:"blur(2px)"}}>
      <div style={{position:"absolute",top:0,left:0,right:0,display:"flex",justifyContent:"center",padding:"max(60px,env(safe-area-inset-top)) 0 0",opacity:textVisible?1:0,transform:textVisible?"translateY(0)":"translateY(-8px)",transition:"opacity .5s ease .1s,transform .5s ease .1s"}}>
      ...
      <div style={{opacity:textVisible?1:0,transform:textVisible?"translateY(0)":"translateY(28px)",transition:"opacity .6s cubic-bezier(.16,1,.3,1) .18s,transform .6s cubic-bezier(.16,1,.3,1) .18s",textAlign:"center",width:"100%"}}>
        <p style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:600,fontSize:"clamp(22px,6.5vw,34px)",color:"#ffffff",lineHeight:1.45,letterSpacing:"-0.01em",margin:0,textShadow:"0 2px 24px rgba(0,0,0,.15)"}}>{text}</p>
        {source==="ai"&&(
...
function StarsShop({ open,onClose,onPurchased,accentRgb,lang,paid }: {
  ...
  return (
    <>
      <div onClick={onClose} style={{position:"absolute",inset:0,zIndex:40,background:"rgba(0,0,0,.25)",opacity:open?1:0,pointerEvents:open?"auto":"none",transition:"opacity .3s",backdropFilter:open?"blur(4px)":"none"}}/>
      <div style={{position:"absolute",bottom:0,left:0,right:0,zIndex:50,borderRadius:"22px 22px 0 0",background:"rgba(255,252,248,.99)",backdropFilter:"blur(32px)",borderTop:`1px solid rgba(${r},${g},${b},.18)`,boxShadow:"0 -14px 50px rgba(0,0,0,.10)",transform:open?"translateY(0)":"translateY(110%)",transition:"transform .42s cubic-bezier(.32,.72,0,1)",paddingBottom:28}}>
        <div style={{display:"flex",justifyContent:"center",paddingTop:13,paddingBottom:5}}>
          <div style={{width:34,height:3,borderRadius:99,background:`rgba(${r},${g},${b},.18)`}}/>
        </div>
        <div style={{textAlign:"center",padding:"8px 24px 22px"}}>
          <p style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:600,fontSize:22,color:"rgba(40,30,50,.90)",margin:0}}>{t.starsTitle}</p>
          <p style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:300,fontSize:13,color:`rgba(${r},${g},${b},.65)`,margin:"8px 0 0"}}>{t.starsSub}</p>
        </div>
        <div style={{padding:"0 20px 14px"}}>
          <button onClick={handleBuy} disabled={loading} style={{display:"flex",alignItems:"center",justifyContent:"space-between",width:"100%",padding:"17px 20px",borderRadius:16,background:`rgba(${r},${g},${b},.07)`,border:`1px solid rgba(${r},${g},${b},.22)`,cursor:loading?"default":"pointer"}}>
            <span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:500,fontSize:17,color:"rgba(40,30,50,.85)"}}>{paid?t.starsPackPaid:t.starsPackFree}</span>
            <span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:600,fontSize:17,color:`rgba(${r},${g},${b},.9)`}}>{loading?"...":(paid?t.starsPricePaid:t.starsPriceFree)}</span>
          </button>
        </div>
        <div style={{padding:"0 20px"}}>
          <button onClick={()=>{playDismiss();onClose();}} style={{width:"100%",padding:"12px 8px",borderRadius:14,background:"transparent",border:"1px solid rgba(40,30,50,.10)",cursor:"pointer",fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:300,fontSize:13,color:"rgba(40,30,50,.32)"}}>{t.starsCancel}</button>
        </div>
      </div>
    </>
  );
...
function HistoryPanel({ entries,open,onClose,accentRgb,lang }: {
  ...
  return (
    <div style={{position:"absolute",inset:0,background:"rgba(20,10,15,.65)",display:"flex",alignItems:"flex-end",zIndex:30,backdropFilter:"blur(6px)"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{width:"100%",background:"#fdf8f5",borderRadius:"24px 24px 0 0",padding:`24px 20px max(24px,env(safe-area-inset-bottom))`,maxHeight:"70dvh",overflowY:"auto"}}>
        ...
        {entries.length===0
          ...
          :[...entries].reverse().map(e=>(
            <div key={e.id} style={{padding:"12px 0",borderBottom:"0.5px solid rgba(40,30,50,.08)"}}>
              ...
              <p style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:300,fontSize:10,color:`rgba(${r},${g},${b},.45)`,margin:"4px 0 0",letterSpacing:"0.08em"}}>{new Date(e.date).toLocaleDateString(lang==="ru"?"ru-RU":"en-US",{month:"short",day:"numeric"})}</p>
            </div>
          ))
        }
...
export default function CategoryScreen({ lang,category,onBack,onCategoryChange,swipeDir }: Props) {
  ...
  return (
    <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}
      style={{position:"fixed",inset:0,background:"#fdf8f5",display:"flex",flexDirection:"column",overflow:"hidden",height,opacity:mounted?1:0,transform:mounted?"translateX(0)":`translateX(${enterX}px)`,transition:mounted?"opacity .32s ease,transform .38s cubic-bezier(.22,1,.36,1)":"none"}}>

      {/* Top bar — 60px min top padding so it clears Telegram system UI */}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",paddingTop:"max(60px,env(safe-area-inset-top))",paddingLeft:20,paddingRight:20,paddingBottom:6,flexShrink:0,position:"relative",zIndex:10}}>
        <button onClick={onBack} style={{background:"transparent",border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:400,fontSize:14,letterSpacing:"0.01em",color:"rgba(40,30,50,.40)",padding:"4px 0",minWidth:56}}>
          {t.backLabel}
        </button>
        ...
        <button onClick={()=>setHistoryOpen(true)} style={{background:"transparent",border:"none",cursor:"pointer",fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:400,fontSize:14,color:"rgba(40,30,50,.40)",padding:"4px 0",minWidth:56,textAlign:"right"}}>
          {t.history}
        </button>
      </div>
      ...
      {/* HeartbeatCanvas — beautiful ripple rings, 40% bigger than original */}
      <div style={{flex:1,position:"relative",minHeight:0}}>
        <HeartbeatCanvas onHoldComplete={handleHoldComplete} isCasting={isCasting} color={cfg} hintText={hintText} holdDuration={2600} baseRScale={0.28}/>
      </div>
