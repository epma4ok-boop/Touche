  ...
  return (
    <BottomSheet>
      ...
      <div style={{ ...rowStyle, cursor: "default" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          {iconBox("⚧️")}
          <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 600, fontSize: 16, color: "rgba(255,238,248,0.90)" }}>{ml.gender}</span>
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          {(["male", "female"] as Gender[]).map((g) => {
            const active = gender === g;
            return (
              <button key={g} onClick={(e) => { e.stopPropagation(); onGenderSwitch?.(g); }}
                style={{
                  padding: "5px 12px", borderRadius: 20, border: `1.5px solid rgba(${PR},${PG},${PB},${active ? 0.7 : 0.22})`,
                  background: active ? `rgba(${PR},${PG},${PB},0.22)` : "transparent",
                  color: active ? `rgba(255,238,248,0.95)` : `rgba(255,238,248,0.40)`,
                  fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: active ? 700 : 400, fontSize: 13,
                  cursor: "pointer", transition: "all .2s",
                  boxShadow: active ? `0 0 10px rgba(${PR},${PG},${PB},0.30)` : "none",
                }}>
