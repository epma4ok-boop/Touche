...
/* ─── Menu labels ──────────────────────────────────────────────── */
const MENU_LABELS: Record<Lang, { menu: string; instructions: string; subscription: string; language: string; about: string }> = {
  ru: { menu: "Меню", instructions: "Инструкция", subscription: "Premium", language: "Язык", about: "О приложении" },
  en: { menu: "Menu", instructions: "How to play", subscription: "Premium", language: "Language", about: "About" },
  hi: { menu: "मेनू", instructions: "कैसे खेलें", subscription: "Premium", language: "भाषा", about: "ऐप के बारे में" },
  pt: { menu: "Menu", instructions: "Como jogar", subscription: "Premium", language: "Idioma", about: "Sobre o app" },
  es: { menu: "Menú", instructions: "Cómo jugar", subscription: "Premium", language: "Idioma", about: "Sobre la app" },
};
...
function MenuPanel({ lang, onClose, onLangSwitch }: { lang: Lang; onClose: () => void; onLangSwitch: () => void }) {
  ...
  return (
    <BottomSheet>
      <div style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 700, fontSize: 20, color: "rgba(255,238,248,0.95)", marginBottom: 22, textAlign: "center" }}>{ml.menu}</div>

      <button style={rowStyle} onClick={() => setSection("instructions")}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>{iconBox("📖")}<span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 600, fontSize: 16, color: "rgba(255,238,248,0.90)" }}>{ml.instructions}</span></div>
        {chevron}
      </button>

      <button style={{ ...rowStyle, background: `rgba(${PR},${PG},${PB},0.10)`, borderColor: `rgba(${PR},${PG},${PB},0.32)` }} onClick={() => setSection("subscription")}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          {iconBox("⭐", true)}
          <div>
            <div style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 600, fontSize: 16, color: "rgba(255,238,248,0.90)" }}>{ml.subscription}</div>
      ...
      <button style={rowStyle} onClick={() => { onLangSwitch(); onClose(); }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          {iconBox("🌐")}
          <span style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 600, fontSize: 16, color: "rgba(255,238,248,0.90)" }}>{ml.language}</span>
        </div>
      ...
      <button style={rowStyle} onClick={onClose}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          {iconBox("💜")}
          <div>
            <div style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 600, fontSize: 16, color: "rgba(255,238,248,0.90)" }}>{ml.about}</div>
            <div style={{ fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: 11, color: "rgba(255,238,248,0.28)", marginTop: 2 }}>Touché v1.0 · 18+</div>
          </div>
        </div>
      </button>

      <button onClick={onClose} style={{ width: "100%", padding: "13px", borderRadius: 16, border: "1px solid rgba(255,238,248,0.07)", background: "rgba(255,238,248,0.03)", color: "rgba(255,238,248,0.30)", fontFamily: "'Plus Jakarta Sans',sans-serif", fontWeight: 500, fontSize: 14, cursor: "pointer", marginTop: 4 }}>
...
export default function Home({
  lang, coupleId, pendingRefUserId,
  onCategorySelect, onScenarioOpen, onLangSwitch,
