  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        marginLeft:14,
        background:`rgba(${r},${g},${b},${pressed?.13:.06})`,
        borderRadius:15,
        border:`1px solid rgba(${r},${g},${b},${pressed?.24:.11})`,
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"center",padding:"0 8px 0 13px",minWidth:0}}>
        ...
        <span style={{
          ...
          color:`rgba(${r},${g},${b},${pressed?.68:.42})`,marginTop:2,
          transition:"color .15s",
          whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",
        }}>{lbl.sub}</span>
      </div>
      ...
      <div style={{flexShrink:0,paddingRight:16}}>
        {cfg.paid ? (
          <div style={{
            ...
            border:`1px solid rgba(${r},${g},${b},${pressed?.44:.20})`,
            background:`rgba(${r},${g},${b},.06)`,
            fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:500,fontSize:10,
            letterSpacing:"0.12em",textTransform:"uppercase",
            color:`rgba(${r},${g},${b},${pressed?.82:.54})`,
            transition:"all .15s",
          }}>★</div>
        ) : (
          <svg viewBox="0 0 16 16" width="13" height="13" fill="none">
            <path d="M6 4 L10 8 L6 12" stroke={`rgba(${r},${g},${b},${pressed?.60:.30})`}
      ...
      <div style={{
        ...
        <svg viewBox="0 0 44 44" fill="none" width="44" height="44">
          <rect x="10" y="13" width="18" height="22" rx="3"
            fill={`rgba(${r},${g},${b},${pressed?.15:.07})`}
            stroke={`rgba(${r},${g},${b},${pressed?.82:.52})`} strokeWidth="1.5"/>
          <rect x="16" y="9" width="18" height="22" rx="3"
            fill={`rgba(${r},${g},${b},${pressed?.18:.09})`}
            stroke={`rgba(${r},${g},${b},${pressed?.82:.52})`} strokeWidth="1.5"/>
          <path d="M20 17 L30 17 M20 21 L27 21"
            stroke={`rgba(${r},${g},${b},${pressed?.62:.36})`} strokeWidth="1.3" strokeLinecap="round"/>
        </svg>
      </div>
      <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"center",padding:"0 8px 0 13px",minWidth:0}}>
        <div style={{display:"flex",alignItems:"baseline",gap:7}}>
          <span style={{
            ...
            whiteSpace:"nowrap",
          }}>{lang==="ru"?"Сценарии":"Scenarios"}</span>
        ...
        <span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:300,fontSize:"clamp(8px,1.8vw,10px)",letterSpacing:"0.14em",textTransform:"uppercase",color:`rgba(${r},${g},${b},${pressed?.68:.42})`,marginTop:2}}>
...
export default function Home({ lang,onCategorySelect,onScenarioOpen }: HomeProps) {
  ...
  // Top inset: read from Telegram API if available, fallback to CSS env
  ...
  useEffect(()=>{
    ...
    function updateMetrics() {
      // Prefer Telegram's own safe area API (v8.0+)
    ...
    // fallback: re-check after expand settles
    const tm=setTimeout(updateMetrics, 400);
    requestAnimationFrame(()=>setMounted(true));
    return()=>{ tg?.offEvent?.("viewportChanged",updateMetrics); clearTimeout(tm); };
  },[]);
  ...
  // Height: use stable viewport height from TG, fallback dvh
  const height=vh?`${vh}px`:"100dvh";

  // Header top padding: TG safe area + minimum 10px; use CSS env as fallback
  const headerPaddingTop = topInset > 0
    ? `${topInset + 10}px`
    : "max(env(safe-area-inset-top), env(safe-area-inset-top, 10px))";
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        paddingLeft:18, paddingRight:18, paddingBottom:9,
        flexShrink:0,
      ...
      {/* Categories — fill remaining height evenly, no gaps */}
      ...
      <div style={{
        flexShrink:0,
        padding:`6px 18px max(12px,env(safe-area-inset-bottom))`,
