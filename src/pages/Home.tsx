...
function useTelegramTopInset(): string {
  ...
  return topPx > 0 ? `${topPx}px` : "max(env(safe-area-inset-top, 0px), 0px)";
}
...
function CategoryCard({
  ...
  const label = lang === "ru"
    ? { main: UI.ru[`catCompliments` === category ? "catCompliments" : category === "tenderness" ? "catTenderness" : category === "desire" ? "catDesire" : category === "passion" ? "catPassion" : "catHard"], sub: CATEGORY_SUBS[category].ru }
    : { main: UI.en[`catCompliments` === category ? "catCompliments" : category === "tenderness" ? "catTenderness" : category === "desire" ? "catDesire" : category === "passion" ? "catPassion" : "catHard"], sub: CATEGORY_SUBS[category].en };

  const catKey = category === "compliments" ? "catCompliments"
    : category === "tenderness" ? "catTenderness"
    : category === "desire" ? "catDesire"
    : category === "passion" ? "catPassion"
    : "catHard";

  const mainLabel = UI[lang][catKey as keyof typeof UI.ru] as string;
  const subLabel = CATEGORY_SUBS[category][lang];
  const levelLabel = LEVEL_LABELS[category][lang === "ru" ? "ru" : "en"];
  ...
  return (
    <button
      ...
      style={{
        ...
        opacity: visible ? 1 : 0,
        transform: visible
          ? `translateY(0) scale(${pressed ? 0.97 : 1})`
          : "translateY(12px)",
