...
export default function ScenarioScreen({ lang, onBack }: ScenarioScreenProps) {
  ...
  useEffect(() => {
    ...
    // Free mode: always unlocked, no subscription check needed
  }, []);
  ...
  const handleHoldComplete = useCallback(async () => {
    ...
    try {
      ...
      const fallbacks: Record<Lang, Record<Intensity, { title: string; role: string }>> = {
        ru: { romantic: { title: "Детектив и свидетель", role: "Задавай партнёру личные вопросы. Только слова — никаких прикосновений. Веди допрос." }, passion: { title: "Фотограф и модель", role: "Снимай партнёра командами, не касаясь. Ищи красоту в каждом взгляде." }, hard: { title: "Хозяин и слуга", role: "Отдавай конкретные смелые приказы. Без объяснений и оправданий." } },
        en: { romantic: { title: "Detective & Witness", role: "Question your partner — words only, no touching. Run the interrogation." }, passion: { title: "Photographer & Model", role: "Direct without touching. Find beauty in every angle." }, hard: { title: "Master & Servant", role: "Give bold specific commands. No explanations needed." } },
        hi: { romantic: { title: "जासूस और गवाह", role: "व्यक्तिगत सवाल पूछें — केवल शब्द, स्पर्श नहीं। पूछताछ चलाएं।" }, passion: { title: "फोटोग्राफर और मॉडल", role: "बिना छुए निर्देश दें। हर कोण में सुंदरता खोजें।" }, hard: { title: "स्वामी और सेवक", role: "साहसी विशिष्ट आदेश दें। कोई स्पष्टीकरण नहीं।" } },
        pt: { romantic: { title: "Detetive e Testemunha", role: "Questione seu parceiro — só palavras, sem toque. Conduza o interrogatório." }, passion: { title: "Fotógrafo e Modelo", role: "Dirija sem tocar. Encontre beleza em cada ângulo." }, hard: { title: "Mestre e Servo", role: "Dê comandos ousados e específicos. Sem explicações." } },
        es: { romantic: { title: "Detective y Testigo", role: "Interroga a tu pareja — solo palabras, sin tocar. Lleva el interrogatorio." }, passion: { title: "Fotógrafo y Modelo", role: "Dirige sin tocar. Encuentra belleza en cada ángulo." }, hard: { title: "Amo y Sirviente", role: "Da órdenes atrevidas y específicas. Sin explicaciones." } },
      };
      const chosen = fallbacks[lang]?.[intensity] ?? fallbacks.en[intensity];
