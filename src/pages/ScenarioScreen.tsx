...
function clearActiveScenario() {
  try { localStorage.removeItem(ACTIVE_SCENARIO_KEY); } catch {}
}
...
function getCurrentUserId(): number | null {
  try { return (window as any).Telegram?.WebApp?.initDataUnsafe?.user?.id ?? null; }
  catch { return null; }
}
...
function ScenarioPaywall({ lang, onSubscribed, onBack }: {
  lang: Lang; onSubscribed: () => void; onBack: () => void;
}) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: BG, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "32px 28px max(40px,env(safe-area-inset-bottom))", position: "relative", zIndex: 1 }}>
        ...
        {/* Title */}
        <p style={{
          fontFamily: "'Plus Jakarta Sans','DM Sans',sans-serif", fontWeight: 700, fontSize: 24,
          color: TEXT_P, margin: "0 0 4px", letterSpacing: "-0.02em", textAlign: "center",
          opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(10px)",
          transition: "opacity .5s ease .15s, transform .5s ease .15s",
        }}>{t.subTitle}</p>

        <p style={{
          fontFamily: "'Plus Jakarta Sans','DM Sans',sans-serif", fontWeight: 300, fontSize: 13,
          letterSpacing: "0.14em", textTransform: "uppercase",
          color: `rgba(${r},${g},${b},0.65)`, margin: "0 0 28px", textAlign: "center",
          opacity: visible ? 1 : 0, transition: "opacity .5s ease .2s",
        }}>{t.subTagline}</p>
        ...
        <div style={{
          ...
          borderRadius: 18, padding: "4px 0", marginBottom: 24,
        ...
        <button
          ...
          style={{
            ...
            fontFamily: "'Plus Jakarta Sans','DM Sans',sans-serif", fontWeight: 600, fontSize: 17,
            color: "rgba(255,220,235,0.95)", letterSpacing: "-0.01em",
            opacity: loading ? 0.6 : (visible ? 1 : 0),
            transform: visible ? "translateY(0)" : "translateY(10px)",
            transition: "opacity .5s ease .35s, transform .5s ease .35s, background .2s",
            marginBottom: 10,
          }}
        >
          {loading ? "..." : t.subBtn}
        ...
        <p style={{ fontFamily: "'Plus Jakarta Sans','DM Sans',sans-serif", fontWeight: 300, fontSize: 11, letterSpacing: "0.08em", color: TEXT_S, margin: "0 0 4px", textAlign: "center" }}>{t.subPriceNote}</p>
      </div>
    </div>
  );
}
...
/* ── Role reveal card ────────────────────────────────────────────────────────*/
function RoleCard({
  title, roleText, intensity, lang, notified, isMissed, onComplete, onHideCard, topPadding,
}: {
  title: string; roleText: string; intensity: Intensity;
  lang: Lang; notified: boolean; isMissed?: boolean;
  ...
  return (
    <div style={{
      ...
      {/* Top row: back button + missed badge */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12, flexShrink: 0 }}>
        <button onClick={onHideCard} style={{ background: "transparent", border: "none", cursor: "pointer", fontFamily: "'Plus Jakarta Sans','DM Sans',sans-serif", fontWeight: 500, fontSize: 14, color: "rgba(255,255,255,0.60)", padding: "4px 0" }}>
          {t.back}
        </button>
        {isMissed && (
          <span style={{ fontFamily: "'Plus Jakarta Sans','DM Sans',sans-serif", fontWeight: 600, fontSize: 9, letterSpacing: "0.18em", textTransform: "uppercase", color: "rgba(255,255,255,0.55)", background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.20)", borderRadius: 20, padding: "3px 9px" }}>
            {lang === "ru" ? "пропущено" : "missed"}
          </span>
        )}
      </div>
      ...
      <div style={{ opacity: textVisible ? 1 : 0, transition: "opacity .5s ease .55s", flexShrink: 0, display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <button onClick={onComplete} style={{ width: "100%", padding: "18px 8px", borderRadius: 18, background: "rgba(255,255,255,0.16)", border: "1px solid rgba(255,255,255,0.30)", cursor: "pointer", fontFamily: "'Plus Jakarta Sans','DM Sans',sans-serif", fontWeight: 600, fontSize: 16, color: "#ffffff" }}>
          {t.complete}
        </button>
      </div>
    </div>
...
export default function ScenarioScreen({ lang, onBack }: ScenarioScreenProps) {
  ...
  useEffect(() => {
    ...
    // Check for a persisted active scenario
    ...
    // ── Subscription check ──────────────────────────────────────────────────
    // Owner bypass: if OWNER_TELEGRAM_ID is set and matches current user, skip check
  ...
  const handleHoldComplete = useCallback(async () => {
    ...
    try {
      ...
      setRevealRoleText(roleText);
      setRevealIntensity(intensity);
      ...
      setRevealIntensity(intensity);
      setNotified(false);
      saveActiveScenario({ sessionId: "", role: "a", roleText: fb.role, title: fb.title, intensity, notified: false });
      setPhase("revealed");
    } finally {
      setIsCasting(false);
      setHintText(T[lang].holdHint);
    }
  }, [isCasting, isLocked, lang, intensity]);

  // "← Назад" on card — hides overlay but keeps card in localStorage
  const handleCardBack = useCallback(() => {
    setPhase("idle");
  }, []);

  // "Задание выполнено" — clears the card permanently and goes back
  ...
  // Show paywall once sub check is done and user is locked
  if (subChecked && isLocked) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: BG, display: "flex", flexDirection: "column", overflow: "hidden", opacity: mounted ? 1 : 0, transition: "opacity .32s ease" }}>
      ...
      {/* Role reveal — shown as overlay; persists until "Task completed" */}
      {phase === "revealed" && (
        <RoleCard
          title={revealTitle}
