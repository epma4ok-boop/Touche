...
/* ── Role reveal card ────────────────────────────────────────────────────────*/
function RoleCard({
  title, roleText, intensity, lang, notified, onComplete, topPadding,
}: {
  title: string; roleText: string; intensity: Intensity;
  lang: Lang; notified: boolean; onComplete: () => void; topPadding: string;
}) {
  const t = T[lang];
...
export default function ScenarioScreen({ lang, onBack }: ScenarioScreenProps) {
  ...
  // "Task completed" — clears the card and goes back to main screen
